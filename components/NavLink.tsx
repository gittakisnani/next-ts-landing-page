...
import { RiArrowUpSLine, RiArrowDownSLine } from "./Icons"
    ...
    return (
        <div className="md:relative bg-inherit">
            <div className="flex gap-2 items-center justify-between">
                ...
                {subLinks &&
                    <>
                    {subLink && <div className="md:absolute bg-inherit top-[140%] z-[1000] left-0 flex flex-col w-max">
                    {width! > 768 && <div className='absolute top-0 right-0 left-0 h-[2px] bg-primary'></div>}
                        {subLinks.map(({text, link, secondSubLinks}, index) => (
                            <SubLink text={text} link={link} secondSubLinks={secondSubLinks} key={index}  />
                        ))}
                    </div>}
                </>
